function [put_price,diff_put,volatility] = BLS_putprice(FTSE,Puts,K,rate)
...
for t= 1:1:(T-window)
    ...
    %the u_i per annum
    volatility(t)= sqrt(sum(u_i.^2)/(n-1)-((sum(u_i)^2)/(n*(n-1))))/sqrt(tau);
    %volatility(t)=volatility_annum(t)*sqrt(tau); % daily?annum?
    ...
    put_price(t)= K*exp(-r(t+window,1)*time)* normcdf(-d2)-S(t+window,1)* normcdf(-d1);
end
