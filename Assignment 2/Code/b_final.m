...
rand=[15,54,60,52,36]; % for convenient, fix the random number
...
for i=1:5
    [Call_est(:,i),diff_call(:,i),volatility_c(:,i)] = BLS_callprice(FTSE100(:,2),Calls(:,i),index(i),FTSE100(:,3)/100);
    [Put_est(:,i),diff_put(:,i),volatility_p(:,i)] = BLS_putprice(FTSE100(:,2),Puts(:,i),index(i),FTSE100(:,3)/100);
    figure(i);
    subplot(2,1,1);
...
diff_call_fill=fillmissing(diff_call,'constant',0);
diff_put_fill=fillmissing(diff_put,'constant',0);
% calculate difference
avg_diff_c=mean(diff_call_fill);
avg_diff_p=mean(diff_put_fill);
...
plot(70:275,diff_call(:,5));
legend(strcat('c',num2str(index(1))),strcat('c',num2str(index(2))),strcat('c',num2str(index(3))),strcat('c',num2str(index(4))),strcat('c',num2str(index(5))));
title('Systematic Difference (True Price - Estimated Price)','Fontsize',16);
ylabel('Difference (Call)','FontSize',14);
subplot(2,1,2);
plot(70:275,diff_put(:,1));
hold on;
plot(70:275,diff_put(:,2));
...
legend(strcat('p',num2str(index(1))),strcat('p',num2str(index(2))),strcat('p',num2str(index(3))),strcat('p',num2str(index(4))),strcat('p',num2str(index(5))));
xlabel('Time Window','FontSize',14);
ylabel('Difference (Put)','FontSize',14);
saveas(gcf,'diff_p.png');